...
public abstract class AbstractAdapter<M, S, J extends JpaRepository<S, UUID>> {
    ...
    protected M mapToDTO(S s) {
        return modelMapper.map(s, valueObjectClass);
    }

    protected List<M> mapToDTO(List<S> s) {
        return s.stream()
                .map(this::mapToDTO)
                .collect(Collectors.toList());
    }

    protected S mapToEntity(M s) {
        return modelMapper.map(s, persistentClass);
    }


    protected List<S> mapToEntity(List<M> s) {
        return s.stream()
                .map(this::mapToEntity)
                .collect(Collectors.toList());
    }

    public M getById(UUID id) throws Exception {
        return mapToDTO(jpaRepository.findById(id).orElseThrow(() -> new Exception("Resource not found whith id : " + id)));
    }
